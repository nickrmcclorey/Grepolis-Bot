...
def open_gui():
    ...
    username_input.grid(row=0, column=2, pady=pady, stick='W')
    ...
    password_input.grid(row=1, column=2, stick='W')
    ...
    Entry(root, width=30, name='max_hours_to_run').grid(row=2, column=2, pady=pady, stick='W')

    Label(root, text="max game sessions").grid(row=3, column=1, pady=pady, stick='W', padx=(0, 10))
    Entry(root, width=30, name='max_sessions').grid(row=3, column=2, pady=pady, stick='W')
    ...
    OptionMenu(root, frequency, *choices).grid(row=4, column=2, stick='W', pady=pady)
    ...
    def run_button_callback(e):
        ...
        cancel_flag = BooleanVar()
        cancel_flag.set(False)
        t = threading.Thread(target=lambda: play_grepolis(cancel_flag))
        t.start()
        # play_grepolis(cancel_flag)
